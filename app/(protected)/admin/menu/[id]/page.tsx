...
export default function EditMenuPage() {
  ...
  const { id } = useParams(); // Retrieves the menu ID from the dynamic route URL
  ...
  // Watch image field to render thumbnail instantly upon update
  const currentImageUrl = form.watch("image");

  // Intercepts input typing to physically reject inputs with > 2 decimals
  ...
  // Fetch Categories and Existing Menu Data
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // 2. Fetch Single Menu Entry Item data
        if (id) {
          const menuResponse = await axiosInstance.get(`/menu/${id}`);
          const menuData = menuResponse?.data?.data || menuResponse?.data;

          if (menuData) {
            // Update react-hook-form state with database values, incorporating incoming image and cooking time fields
            form.reset({
              title: menuData.title || "",
              price: menuData.price || 0,
              cookingTime: menuData.cookingTime || 0,
              categoryId: menuData.categoryId?._id || menuData.categoryId || "",
              image: menuData.image || "",
              ingredientItem: menuData.ingredientItem?.length ? menuData.ingredientItem : [""],
              addOnItems: menuData.addOnItems || [],
            });
          }
        }
      } catch (error) {
  ...
  const onSubmit = async (values: MenuFormValues) => {
    try {
      ...
      await axiosInstance.patch(`/menu/${id}`, values);
  ...
  const handleUploadComplete = (data: any) => {
    setUploadOpen(false);
    // Standardize URL fallback based on your backend response configuration
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {/* Hidden field storing image state URL for payload compilation */}
            <FormField
              control={form.control}
              name="image"
          ...
          {/* Dynamic Ingredients Section - REQUIRED */}
          ...
          {/* Dynamic Add-on Items Section (Strict Filtering) */}
          <div className="space-y-3 bg-black/[0.01] p-5 border border-black/10 rounded-xl">
            <div className="flex items-center justify-between">
          ...
          <div className="flex justify-end gap-3 pt-5">
            ...
            >
              {submitLoading ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : "Save Changes"}
            </Button>
          </div>
        </form>
      </Form>
