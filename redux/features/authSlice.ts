...
>('auth/login', async (userCredentials, { rejectWithValue }) => {
  try {
    ...
    localStorage.setItem('pattybros', JSON.stringify(response.data.accessToken));
    return response;
  } catch (error: any) {
...
>('auth/google', async (googleUserCredentials) => {
  ...
  const response = await request.data;
  localStorage.setItem('pattybros', JSON.stringify(response.data.access_token));
  return response;
});
// forgot password
...
const authSlice = createSlice({
  ...
  reducers: {
    ...
    logout(state) {
      ...
      state.token = null;
      state.error = null;
      localStorage.removeItem('pattybros');
    },
