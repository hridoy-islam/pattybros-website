...
export default function ReservationDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (data.tableId) setSelectedTableId(data.tableId._id);
 ...
 useEffect(() => {
  ...
  const fetchTables = async () => {
    try {
      const res = await axiosInstance.get("/table", {
        params: { status: "active", limit: "all" },
      });
      const data: ITable[] = res?.data?.data?.result || res?.data?.data || res?.data || [];
      
      // Check if the reservation already has a table assigned
      const currentAssignedTable = reservation.tableId;

      if (currentAssignedTable) {
        // Look if the assigned table is already present in the active list
        const isTableAlreadyListed = data.some((t) => t._id === currentAssignedTable._id);
        
        if (!isTableAlreadyListed) {
          // Explicitly map properties to match the ITable interface
          const formattedAssignedTable: ITable = {
            _id: currentAssignedTable._id,
            tableNumber: currentAssignedTable.tableNumber,
            capacity: currentAssignedTable.capacity,
            location: currentAssignedTable.location,
            status: "active", // Force state status as active for frontend selection compatibility
          };
          
          // Merge it into the state list
          setAllTables([formattedAssignedTable, ...data]);
          return;
        }
      }
  ...
  const eligibleTables = allTables
    .filter((table) => table.capacity >= (reservation?.partySize || 0) || table._id === reservation?.tableId?._id)
    .sort((a, b) => a.capacity - b.capacity);

  // ── Trigger Confirm Dialog ─────────────────────────────────────────────────
  const checkConfirmAssignment = (values: IFormInputs) => {
    if (!selectedTableId) return;
    const matchedTable = allTables.find((t) => t._id === selectedTableId);
    const isUpdate = reservation?.status === "confirmed";

    setConfirmConfig({
      isOpen: true,
      title: isUpdate ? "Update Table Assignment?" : "Confirm & Assign Table?",
      description: `Are you sure you want to allot Table ${matchedTable?.tableNumber || ""} (${matchedTable?.capacity} seats) to this reservation request?`,
      actionButtonText: isUpdate ? "Update Assignment" : "Confirm Assignment",
    ...
    try {
      ...
      setReservation((prev) =>
        prev
          ? {
              ...prev,
              tableId: allTables.find((t) => t._id === selectedTableId),
              startTime: values.startTime || undefined,
              endTime: values.endTime || undefined,
              status: "confirmed",
            }
      ...
      toast({
        ...
        description: "Table allocation applied successfully.",
      });
    } catch (err: any) {
  ...
  const triggerCompleteDialog = () => {
    setConfirmConfig({
      ...
      description: "This structural update locks the record and returns the allocated table back into the active general pool.",
  ...
  const executeComplete = async () => {
    try {
      ...
      setAllTables((prev) =>
        prev.map((t) => (t._id === selectedTableId ? { ...t, status: "active" } : t))
      );

      toast({
        title: "Reservation Completed",
        description: "Status marked as completed. Table is now active.",
      });
    } catch (err: any) {
  ...
  const isFormReady = !!selectedTableId;
  ...
  return (
    <div className="mx-auto space-y-6 text-black">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        {/* ── LEFT: Guest Information Metadata Card (Sticky & Content-Fit Height) ── */}
        <div className="lg:sticky lg:top-6 h-fit border border-black/10 rounded-xl p-5 space-y-1 bg-white shadow-sm">
          ...
          {isConfirmed && reservation.tableId && (
            <div className="mt-4 p-3 bg-emerald-50 border border-emerald-200 rounded-lg">
              <div className="flex items-center gap-2 text-emerald-800 font-semibold text-sm">
                <CheckCircle2 className="w-4 h-4 text-emerald-600" />
                Assigned to {reservation.tableId.tableNumber} ({reservation.tableId.capacity} seats)
              </div>
          ...
          {isCompleted && reservation.tableId && (
            <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <div className="flex items-center gap-2 text-blue-800 font-semibold text-sm">
                <CheckCircle2 className="w-4 h-4 text-blue-600" />
                Completed on Table {reservation.tableId.tableNumber}
              </div>
            </div>
          )}
        </div>

        {/* ── RIGHT: Table Assignment Operations Card ── */}
        <div className="border border-black/10 rounded-xl p-5 bg-white space-y-5 shadow-sm">
          ...
          {/* Tables Selection */}
          <div>
            <Label className="text-xs font-semibold uppercase tracking-wide text-black/50 mb-2 block">
              Eligible Tables Match List (Capacity &ge; {reservation.partySize})
            </Label>
            {eligibleTables.length === 0 ? (
              <div className="flex items-center justify-center border border-dashed border-rose-200 bg-rose-50/40 rounded-lg py-8 text-rose-700 text-sm">
                No active tables available for this group size.
              </div>
            ) : (
                ...
                {eligibleTables.map((table) => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative flex flex-col items-center justify-center p-3 rounded-lg border-2 transition-all text-center ${
                        isDisabled ? "opacity-50 cursor-not-allowed" : "cursor-pointer"
                      } ${isSelected ? "border-primary bg-primary text-white" : "border-black/15 bg-white text-black"}`}
                    >
                      <TableProperties className={`w-5 h-5 mb-1 ${isSelected ? "text-white" : "text-black/40"}`} />
            ...
            <form onSubmit={form.handleSubmit(checkConfirmAssignment)} className="space-y-5">
              ...
              <div className="space-y-2 pt-2 border-t border-black/5">
                {!isCompleted && !isCancelled && (
                  <Button
                    type="submit"
                    disabled={!isFormReady || submitLoading}
                  ...
                  >
                    {submitLoading ? (
                      ...
                      <div className="flex items-center justify-center gap-2">
                        {isConfirmed ? "Update Layout" : "Confirm & Assign Table"}
                      </div>
                    )}
                  </Button>
                )}
      ...
      >
        <AlertDialogContent className="bg-white text-black border border-black/10">
          ...
          <AlertDialogFooter className="gap-2 sm:gap-0">
            <AlertDialogCancel className="border border-black/10 hover:bg-black/90">
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
